...
| Scanner raises `UnsupportedInputError` as a domain precondition guard when bars contain non-universe symbols | Forcing `unsupported-input` rejections onto every universe symbol | Run-level fail-closed without corrupting per-symbol decision semantics; review finding REL-001 showed batch-wide forced rejections silently discarded valid signals. For real fixtures the spec-mandated loader check rejects unknown symbols first as `fixture-symbol-missing` (CLI-observable path, pinned by test); the scanner raise and its CLI `unsupported-input` mapping are defense in depth for future non-fixture callers (REL/FREL findings). |
