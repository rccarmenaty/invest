...
- [ ] 1.1 **RED** Extend `tests/domain/test_market_context.py` for immutable `GenerationSpan(start, end)`, inversion, and fail-closed out-of-span status/completeness/eligibility.
- [ ] 1.2 **GREEN/REFACTOR** Add required `GenerationSpan` and span authority in `src/invest/domain/market_context.py`.
- [ ] 1.3 **RED** Extend `tests/domain/test_market_context_builder.py` for exact first/last-session propagation and empty-span rejection.
- [ ] 1.4 **GREEN/REFACTOR** Propagate normalized session bounds in `src/invest/domain/market_context_builder.py` and `src/invest/application/generate_market_context.py`.

## 2. Versioned context JSON

- [ ] 2.1 **RED** Extend `tests/adapters/test_backtest_context_json.py` for deterministic `market-context-v2` round-trip and rejection of v1, missing, malformed, empty, or inverted spans.
- [ ] 2.2 **GREEN/REFACTOR** Require canonical top-level `generation_span` in `src/invest/adapters/backtest_context_json.py`; no inference or compatibility fallback.
