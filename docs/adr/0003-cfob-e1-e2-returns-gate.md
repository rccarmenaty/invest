...
- **One shared, fully-supported placebo pool (clarified — amendment A1):** admissibility is the E1 forward-window/embargo rule **intersected with full E2 support** (a computable benchmark residual: complete forward focal+factor window **and** an estimable 252-session pre-date beta). E1 and E2 draw from this **single identical pool**, so both legs are defined on every draw. The `< 100 admissible → excluded` gate above is therefore applied to the **E2-supported** pool **before inference** — a cluster is never dropped after the fact for a placebo it cannot evaluate. (Support is a property of admissibility, not a post-sampling filter; the earlier build's post-hoc "any unsupported placebo → drop cluster" was a deviation from this section, corrected 2026-07-24.)
- **Price-panel warm-up (amendment A1):** the adjusted-price panel and the derived daily habitat aggregate must carry **enough history before the first research date** that both the ≥50-name habitat factor and the 252-session pre-date beta are fully constructible there (~2 trading years: 252 factor-floor + 252 beta, plus margin). Without it, the panel's own first ~year is a factor dead-zone (no PIT-eligible names until the floor is met), and within-ticker placebos landing there are needlessly inadmissible. The panel is pulled from `FIRST_YEAR − 4` for this reason. This changes **no estimator** — only how far back the input data is sourced.
...
## Amendment A1 (2026-07-24) — shared E2-supported placebo pool + panel warm-up

The first measured `--measure-returns` run (SEP panel of 7,117 cluster symbols, initially pulled `FIRST_YEAR−2`) returned **`underpowered_stop`, common cohort = 12** — a **mechanical** collapse, not a returns finding. Root cause, measured: the habitat aggregate could not warm up until the 252-bar floor was met, so the panel's own first year was a **factor dead-zone** (251 contiguous NaN factor sessions at the front of every focal array). Within-ticker placebos landing in that dead-zone — or with a 252-session beta window reaching into it — were E2-inadmissible, and the build's post-hoc rule (drop the whole cluster if **any** of its 100 placebos was unsupported) compounded a ~13% per-placebo miss into 99.6% cohort loss. Focal histories were ample (median 4,433 sessions); the loss was not data scarcity.

This amendment is a **fidelity correction, not a loosening**: §5 always required placebos to carry "the same beta/breadth/forward-window support" and to gate at `< 100 admissible`. A1 makes that operational — admissibility **includes** E2 support (one shared E1/E2 pool, resolved before inference), and the panel is pulled with enough warm-up (`FIRST_YEAR − 4`) that the factor and 252-session beta exist before the first research date. No estimator, constant, cost, α, or bootstrap changed. Because it touches only data sourcing and the admissibility/sampling mechanics the section already specified, it is a correction under this ADR, **not** a new trial against the deflated-Sharpe standard.
